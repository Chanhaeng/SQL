...
get_ipython().run_cell_magic('sql', '', 'SELECT gender, breed_group, COUNT(DISTINCT dog_guid) AS dog\nFROM dogs\nGROUP BY breed_group, gender\nORDER BY dog DESC;')
...
get_ipython().run_cell_magic('sql', '', 'SELECT breed_type, AVG(mean_iti_minutes) AS avg_time, AVG(total_tests_completed) AS avg_test\nFROM dogs\nGROUP BY breed_type\nORDER BY avg_test DESC\n;')
...
get_ipython().run_cell_magic('sql', '', 'SELECT AVG(TIMESTAMPDIFF(minute, start_time, end_time)) AS avg\nFROM exam_answers\nWHERE TIMESTAMPDIFF(minute,start_time, end_time) < 6000*60 AND TIMESTAMPDIFF(minute,start_time, end_time) > 0\nGROUP BY test_name\nORDER BY avg DESC;')
...
get_ipython().run_cell_magic('sql', '', "SELECT state, zip, COUNT(DISTINCT user_guid) as uni_user\nFROM users\nWHERE country='US'\nGROUP BY state, zip\nORDER BY state, zip desc;")
