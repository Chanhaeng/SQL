...
get_ipython().run_cell_magic('sql', '', 'SELECT IF(a.country="US","US",\'outside of the US\') AS US, count(*) AS count\nFROM (SELECT DISTINCT u.user_guid, u.country \n      FROM users u\n      WHERE u.country IS NOT NULL AND u.user_guid IS NOT NULL) a\nGROUP BY US;')
...
get_ipython().run_cell_magic('sql', '', 'SELECT d.dog_guid AS dog_guid, d.breed_type AS breed_type, t.test_name AS test_name, COUNT(*) AS number_of_completed_tests,\n       CASE breed_type\n       WHEN "Pure Breed" THEN "Pure_Bread"\n       ELSE "Not_Pure_Breed"\n       END AS Pure_Breed\nFROM complete_tests t, dogs d\nWHERE d.dog_guid=t.dog_guid\nGROUP BY dog_guid, breed_type, Pure_Breed;')
