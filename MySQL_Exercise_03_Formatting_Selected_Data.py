...
non_us_customers = get_ipython().magic("sql SELECT DISTINCT country FROM users WHERE country!='US'")
non_us_customers.csv('non_us')
...
Q7 = get_ipython().magic("sql SELECT DISTINCT user_guid, membership_type FROM users WHERE membership_type=2 AND state='NC' AND created_at>'2014_03_01' AND country='US' ORDER BY created_at DESC")
...
get_ipython().run_cell_magic('sql', '', 'SELECT UPPER(breed)\nFROM dogs\nORDER BY breed\nLIMIT 6;')
