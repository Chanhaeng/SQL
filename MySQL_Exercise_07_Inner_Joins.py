...
get_ipython().run_cell_magic('sql', '', "SELECT d.user_guid, d.dog_guid, d.breed, d.breed_type, d.breed_group\nFROM complete_tests c, dogs d\nWHERE c.dog_guid = d.dog_guid AND c.test_name = 'Yawn Warm-up'")
...
get_ipython().run_cell_magic('sql', '', "SELECT distinct d.user_guid, u.membership_type, d.dog_guid\nFROM complete_tests c, users u, dogs d\nWHERE d.dog_guid = c.dog_guid \n    AND d.user_guid = u.user_guid\n    AND d.breed = 'golden retriever'\n    ")
...
get_ipython().run_cell_magic('sql', '', "SELECT COUNT(distinct d.dog_guid)\nFROM dogs d, users u\nWHERE d.user_guid = u.user_guid AND d.breed = 'golden retriever' AND u.state = 'NC'")
